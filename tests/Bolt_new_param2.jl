...
md"""
Here, we run some tests to observe how the new energy density ($$Ω_{new}$$) affects results
"""

# ╔═╡ 8f150cf5-0cda-4cd3-b77b-f89cd892ead3
md"""
Starting with the base case, where $$Ω_{new}$$ = 0 (as before) :
"""
...
$$Ω_{new} = Ae^{\frac{(a-a_{peak})^2}{2c^2}}$$

A = 5

a_peak = 0.000833

The standard deviation (c) will be varied
"""
...
begin
	plot(ℓs, (@. ℓs^2 * Cᵀᵀ), label="Ω_new = 0", xlabel=L"\ell", ylabel=L"\ell^2 C^{TT}(\ell)", legend=:topright, title= "Power Spectrum of Temperature Fluctuations")
...
plot(bg.x_grid, bg.ℋ, xlabel=L"\log(a)", ylabel=L"\mathcal{H}", label="Ω_new = 0", yscale=:log10, legend=:topright, title="Hubble Parameter")
